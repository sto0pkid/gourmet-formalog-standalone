...
sub DoQuery {
  ...
  my $res1 = $self->TranslateResultBack
    (
     Results => \@allresults,
     ResultType => $args{ResultType} || 'UniLangProxy',
    );
...
sub TranslateResultBack {
  ...
  my @results;
  if ($args{ResultType} eq 'UniLangProxy') {
    push @results, '_prolog_list';
  }
  foreach my $r (@{$args{Results}}) {
    my $bindings = $r->{MyResult} || $r->{MYRESULT};
    # delete $r->{MyResult};
    my $vars = $bindings->[0];
...
sub ConvertJavaPengineToInterlingua {
  ...
  if ($type eq 'ARRAY') {
	...
	return
      ...
      } elsif ($f eq '$COMPOUND_ZERO_ARITY') {
	# FIX COMPOUND
	return
	  {
	   Success => 1,
